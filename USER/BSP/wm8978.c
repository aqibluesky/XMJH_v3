...
uint8_t WM8978_WriteReg(uint8_t RegAddr, uint16_t Value)
{
	...
	IIC_SendByte(WM8978_Addr | IIC_WR);
	//等待响应
	Ack = IIC_WaitAck();
	if (Ack == 1)
	{
	...
	IIC_SendByte(((RegAddr << 1) & 0xFE) | ((Value >> 8) & 0x1));
	Ack = IIC_WaitAck();
	if (Ack == 1)
	{
		return 0;
	}
	IIC_SendByte(Value & 0xFF);
	Ack = IIC_WaitAck();
	if (Ack == 1)
	{
		return 0;
...
{
	IIC_Init(WM8978_Addr);
	WM8978_WriteReg(0,0x0000);	//向0寄存器写入任意值，就可以软件复位
	WM8978_WriteReg(1,0x001B);	//R1,MICEN设置为1(MIC使能),BIASEN设置为1(模拟器工作),VMIDSEL[1:0]设置为:11(5K)
	WM8978_WriteReg(2,0x01B0);	//R2,ROUT1,LOUT1输出使能(耳机可以工作),BOOSTENR,BOOSTENL使能
	WM8978_WriteReg(3,0x006C);	//R3,LOUT2,ROUT2输出使能(喇叭工作),RMIX,LMIX使能
	WM8978_WriteReg(6,0x0000);	//R6,MCLK由外部提供

	WM8978_WriteReg(43,1<<4);	//R43,INVROUT2反向,驱动喇叭
	WM8978_WriteReg(47,1<<8);	//R47设置,PGABOOSTL,左通道MIC获得20倍增益
	WM8978_WriteReg(48,1<<8);	//R48设置,PGABOOSTR,右通道MIC获得20倍增益
	WM8978_WriteReg(49,1<<1);	//R49,TSDEN,开启过热保护
	WM8978_WriteReg(10,1<<3);	//R10,SOFTMUTE关闭,128x采样,最佳SNR
	WM8978_WriteReg(14,1<<3);	//R14,ADC 128x采样率
}
